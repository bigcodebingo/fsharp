:-dynamic high/2.
 ...
 pr2:-tell('C:/Prolog_labs/111.txt'),high_r(X,_),X=nil,told.
 pr3:-see('C:/Prolog_labs/111.txt'),get0(Sym),read_high(Sym),seen.
 read_high(-1):-!.
...
popularity(ruby,1).
popularity(c_sharp,1).
popularity(python,1).
popularity(c_plu_plus,0).
popularity(f_sharp,0).
popularity(prolog,0).
popularity(c,0).
popularity(asm,0).
popularity(javascript,1).
popularity(java,1).
popularity(php,1).
popularity(swift,1).
...
question8(X8):- write("Is your language very popular now?"),nl,
                write("1. Yes"),nl,
				write("0. NO"),nl,
                read(X8).
